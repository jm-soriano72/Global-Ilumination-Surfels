...
void SSAOCompositionPipeline::createGraphicsPipeline(VkDevice device, VkExtent2D swapChainExtent, VkDescriptorSetLayout descriptorSetLayout, VkRenderPass renderPass)
{
    ...
    if (renderConfig == RenderMode::SSAO) fragmentShaderCode = ShaderStagesCreator::readFile(RESOURCES_PATH "shaders/ssao_composition_frag.spv");
    else if (renderConfig == RenderMode::SSAO_SHADOW_MAPPING_PCF || renderConfig == RenderMode::SURFELS_GLOBAL_ILLUMINATION || renderConfig == RenderMode::SURFELS_VISUALIZATION || renderConfig == RenderMode::SURFELS_RADIANCE_VISUALIZATION) fragmentShaderCode = ShaderStagesCreator::readFile(RESOURCES_PATH "shaders/shadow_ssao_composition_frag.spv");
