...
void SurfelsCompositionPipeline::createGraphicsPipeline(VkDevice device, VkExtent2D swapChainExtent, VkDescriptorSetLayout descriptorSetLayout, VkRenderPass renderPass)
{
    ...
    if (renderConfig == RenderMode::SURFELS_GLOBAL_ILLUMINATION) fragmentShaderCode = ShaderStagesCreator::readFile(RESOURCES_PATH "shaders/surfels_composition_frag.spv");
    else if (renderConfig == RenderMode::SURFELS_VISUALIZATION || renderConfig == RenderMode::SURFELS_RADIANCE_VISUALIZATION) fragmentShaderCode = ShaderStagesCreator::readFile(RESOURCES_PATH "shaders/surfels_composition_visualization_frag.spv");
