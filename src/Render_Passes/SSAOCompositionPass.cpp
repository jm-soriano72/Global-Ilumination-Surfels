...
SSAOCompositionPass::SSAOCompositionPass() {}

void SSAOCompositionPass::createRenderPass(VkDevice device, VkFormat format, VkPhysicalDevice physicalDevice)
{
    // Render Pass por defecto, para mostrar el resultado del efecto de SSAO
...
void SSAOCompositionPass::createFramebuffers(uint32_t numImageViews, SwapChainManager swapChainManager, VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool,
...
void SSAOCompositionPass::cleanup(VkDevice device)
{
...
void SSAOCompositionPass::cleanupFramebuffers(VkDevice device) 
{
	depthBufferCreator.cleanupDepthResources(device);
    for (auto &framebuffer : framebuffers)
    {
