...
layout (binding = 9) uniform sampler2D indirectDiffuseMap;
...
vec3 shadeMainLight() {
    ...
    // LUZ INDIRECTA //
    // Se utiliza el mapa de iluminación difusa indirecta
    // Se añade la oclusión ambiental
    vec3 indirectDiffuseIllumination = indirectDiffuse * albedo.rgb * ssao;
    c += indirectDiffuseIllumination * INDIRECT_DIFFUSE_ILLUMINATION_WEIGHT;
...
vec3 sampleIndirectDiffuse(vec2 uv) {
    vec2 texelSize = 1.0 / vec2(textureSize(indirectDiffuseMap, 0));
    ...
    for (int x = -1; x <= 1; ++x) {
        for (int y = -1; y <= 1; ++y) {
            ...
            result += texture(indirectDiffuseMap, uv + offset).rgb;
            count++;
        }
    }
...
void main() 
{
	...
	indirectDiffuse = sampleIndirectDiffuse(inUV);
